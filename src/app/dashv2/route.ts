...
body{font-family:var(--font);color:var(--text);display:flex;flex-direction:column;height:100vh}
...
.hdr-center{position:absolute;left:50%;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:1px}
.hdr-title{font-size:.65rem;font-weight:600;color:var(--text3);letter-spacing:3px;text-transform:uppercase}
.hdr-sub{font-size:.78rem;font-weight:500;color:var(--text2)}
.yr-wrap{display:flex;align-items:center;gap:10px}
.yr-label{font-size:.62rem;font-weight:600;color:var(--text3);letter-spacing:2px;text-transform:uppercase}
.yr-sel{
  background:var(--bg3);border:1px solid var(--border2);color:var(--text);
  padding:5px 28px 5px 12px;border-radius:var(--radius-sm);
  font-family:var(--font);font-size:.78rem;font-weight:500;cursor:pointer;outline:none;
  appearance:none;
...
.tabs{
  ...
  display:flex;gap:2px;padding:8px 20px;flex-shrink:0;
}
.tab{
  ...
  transition:all .2s;font-family:var(--font);display:flex;align-items:center;gap:6px;
  letter-spacing:.3px;
}
...
/* ── SECTION LABEL ── */
.sec-label{
  font-size:.6rem;font-weight:600;color:var(--text3);letter-spacing:3px;
  text-transform:uppercase;padding:0 2px;flex-shrink:0;
...
.card-title{font-size:.65rem;font-weight:600;color:var(--text3);letter-spacing:2px;text-transform:uppercase}
...
.tip-btn{
  ...
  font-size:.55rem;font-weight:700;display:inline-flex;align-items:center;
  justify-content:center;cursor:pointer;position:relative;flex-shrink:0;font-family:var(--font);
}
.tip-btn .tip{
  display:none;position:absolute;top:22px;right:0;z-index:500;
  background:var(--bg3);color:var(--text2);padding:10px 12px;
  border-radius:var(--radius-sm);font-size:.67rem;font-weight:400;
  width:220px;line-height:1.6;letter-spacing:0;text-transform:none;
...
/* ── OVERVIEW LAYOUT ── */
.ov-grid{display:grid;grid-template-columns:1fr 1fr 200px;grid-template-rows:1fr 1fr;gap:10px;flex:1;min-height:0}
.ov-charts-top{display:contents}
...
.kpi-label{font-size:.6rem;font-weight:600;color:var(--text3);letter-spacing:2px;text-transform:uppercase}
.kpi-value{font-size:2.2rem;font-weight:700;color:var(--teal);line-height:1;letter-spacing:-1px;font-variant-numeric:tabular-nums}
.kpi-sub{font-size:.7rem;font-weight:500;color:var(--text2);margin-top:2px}
.kpi-sub-label{font-size:.58rem;color:var(--text3);letter-spacing:1px;text-transform:uppercase;margin-top:1px}
...
.map-panel{
  width:120px;flex-shrink:0;background:var(--bg2);border-left:1px solid var(--border);
  padding:10px 8px;display:flex;flex-direction:column;gap:6px;overflow-y:auto;
  border-radius:0 0 var(--radius) 0;
}
.map-panel-label{font-size:.58rem;font-weight:600;color:var(--text3);letter-spacing:2px;text-transform:uppercase;padding:0 2px}
.reg-btn{
  display:block;width:100%;text-align:left;padding:5px 8px;
  border-radius:var(--radius-sm);border:1px solid var(--border);
  background:transparent;font-family:var(--font);font-size:.68rem;
...
.est-sel{
  width:100%;padding:5px 6px;border:1px solid var(--border);border-radius:var(--radius-sm);
  font-family:var(--font);font-size:.68rem;outline:none;
...
.ft thead th{
  background:var(--bg3);color:var(--text3);padding:8px 10px;
  text-align:center;font-weight:600;letter-spacing:1px;text-transform:uppercase;
  white-space:nowrap;position:sticky;top:0;border-bottom:1px solid var(--border);font-size:.6rem;
}
...
.ft tfoot td{
  background:var(--bg3);font-weight:600;padding:7px 10px;
  border-top:1px solid var(--teal-dim);color:var(--text);
}
...
.gcat{
  font-size:.58rem;font-weight:600;color:var(--teal);letter-spacing:3px;
  text-transform:uppercase;padding:4px 0;
}
.gt{border-collapse:collapse;width:100%;font-size:.67rem}
.gt thead th{
  background:var(--bg3);color:var(--text3);padding:7px 12px;
  text-align:left;font-weight:600;letter-spacing:1px;text-transform:uppercase;
  position:sticky;top:0;border-bottom:1px solid var(--border);font-size:.6rem;
}
...
<nav class="tabs">
  <button class="tab active" onclick="showPg('overview',this)">
    <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
    Overview
  </button>
...
<div class="pg" id="pg-financeiro">
  <div class="fin-layout">
    <div class="fin-table-wrap">
      <table class="ft">
        <thead><tr><th>Ano</th><th>Tri</th><th>Mês</th><th>País</th><th>Churn</th><th>Novos</th><th>Qt Trans</th><th>Custo</th><th>Custo Total</th><th>Invest. Mkt</th><th>Lucro Op.</th><th>Margem %</th><th>Receita</th></tr></thead>
        <tbody id="fB"></tbody>
...
<div class="pg" id="pg-glossario">
  <div class="gl-scroll">
    <div class="card"><div class="card-body">
      ...
      <table class="gt"><thead><tr><th>Medida</th><th>O que é?</th><th>Como é calculada?</th><th>Referência Saudável</th></tr></thead><tbody>
...
const COLORS={teal:'#2dd4bf',tealDim:'rgba(45,212,191,0.7)',orange:'#fb923c',green:'#34d399',navy:'#0d1117',text:'#94a3b8',grid:'rgba(45,57,72,0.8)'};
...
const baseChartOpts=(yFmt)=>({
  ...
  plugins:{
    ...
    datalabels:{
      ...
      formatter:(v)=>'R$'+fK(v),
      color:COLORS.text,font:{size:8,weight:'600',family:'DM Mono'},
      offset:2
    }
  },
  scales:{
    y:{
      ...
      grid:{color:COLORS.grid},border:{color:'transparent'}
    },
    x:{
      grid:{display:false},
...
function byYear(arr,field){
  ...
  if(y!=='all'){
    const d=ft(arr);
    return{labels:d.map(r=>r.mes),datasets:[{label:String(y),data:d.map(r=>r[field]),backgroundColor:AC[Number(y)]||COLORS.teal,borderRadius:4,borderSkipped:false}]};
  }
  const years=[2024,2025,2026];
  const ds=years.map(a=>{const d=arr.filter(r=>r.ano===a);return d.length?{label:String(a),data:d.map(r=>r[field]),backgroundColor:AC[a],borderRadius:4,borderSkipped:false}:null;}).filter(Boolean);
...
function bCusto(){
  ...
  CH.c=new Chart(document.getElementById('cC'),{type:'bar',data:{labels,datasets},options:baseChartOpts(v=>'R$'+fK(v))});
...
function bLucro(){
  ...
  CH.l=new Chart(document.getElementById('lC'),{type:'bar',data:{labels,datasets},options:baseChartOpts(v=>'R$'+fK(v))});
...
function filterRegiao(r,btn){
  ...
  btn.classList.add('active');renderMarkers();
}
...
function renderFin(){
  ...
  CH.fc=new Chart(document.getElementById('fCC'),{
    data:{labels:lbls,datasets:[
      {type:'line',label:'Custo',data:cs,borderColor:COLORS.teal,backgroundColor:'rgba(45,212,191,.08)',fill:true,tension:.4,pointRadius:3,pointBackgroundColor:COLORS.teal,pointBorderColor:COLORS.navy,pointBorderWidth:2},
      {type:'line',label:'Tendência',data:tr,borderColor:COLORS.text,borderDash:[5,4],borderWidth:1.5,pointRadius:0,fill:false}
    ]},
    options:{
      responsive:true,maintainAspectRatio:false,
      plugins:{
        legend:{position:'bottom',labels:{font:{family:'DM Sans',size:10},color:COLORS.text,boxWidth:10,boxHeight:10,padding:12}},
        datalabels:{display:false}
      },
      scales:{
        y:{ticks:{callback:v=>'R$'+fK(v),font:{size:9,family:'DM Mono'},color:COLORS.text},grid:{color:COLORS.grid},border:{color:'transparent'}},
        x:{grid:{display:false},ticks:{font:{size:9},color:COLORS.text},border:{color:COLORS.grid}}
      }
    }
  });
}
